...
class _AdvancedShapeBase(pyglet.shapes._ShapeBase):
    ...
    def _update_anchor_position(self):
        if self._anchor_visible:
            ...
            self._anchor_circle.position = (self._x, self._y)
    ...
    def _update_anchor_color(self):
        if self._anchor_circle is not None:
            self._anchor_circle._rgb = (255 - c for c in self._rgb)
            self._anchor_circle._opacity = self._opacity
            self._anchor_circle._update_color()
    ...
    def _translate_vertices(self):
        for i in range(0, len(self._vertices), 2):
            self._vertices[i] += self._x - self._anchor_x
            self._vertices[i + 1] += self._y - self._anchor_y
...
class Circle(_AdvancedShapeBase):
    def __init__(self, x, y, radius, rotation=0, segments=None, color=(255, 255, 255), opacity=255,
    ...
    def _calculate_vertices(self):
        ...
        points = [[r * math.cos(i * tau_segs), r * math.sin(i * tau_segs)] for i in range(self._segments)]
...
if __name__ == "__main__":
    ...
    shape = Circle(window.width / 2, window.height / 2, 200, segments=9, color=color_body, batch=batch)
    # shape = Rectangle(window.width / 2, window.height / 2, 200, 300, color=color_body, batch=batch)
