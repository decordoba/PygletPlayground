...
if __name__ == "__main__":
    ...
    shape = Arc(window.width / 2, window.height / 2, 300, start_angle=90, angle=90, color=color_body, batch=batch)
    ...
    def update(dt):
        ...
        if direction is None:
            ...
            if int(c[0]) == 0 and int(c[2]) == 255:
                direction = "up"
            else:
                shape.color = (max(c[0] - speed, 0), 0, min(c[2] + speed, 255))
        elif direction == "up":
            if int(c[0]) == 255 and int(c[2]) == 0:
                direction = None
            else:
                shape.color = (min(c[0] + speed, 255), 0, max(c[2] - speed, 0))
