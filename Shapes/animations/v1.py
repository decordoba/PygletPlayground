...
class Circle(_AdvancedShapeBase):
    ...
    def closed(self):
        ...
        This will not be noticable in a circle
...
if __name__ == "__main__":
    ...
    color_body = (180, 23, 47)
    color_line = (255, 255, 0)
    middle_x, middle_y = window.width / 2, window.height / 2
    shape = Circle(window.width / 4, window.height / 2, 200, segments=6, closed=True, color=color_body, batch=batch)
    shape2 = Circle(window.width / 4 * 3, window.height / 2, 200, segments=6, closed=False, color=invert_color(color_body), batch=batch)
    ...
    def update(dt):
        ...
        shape2.angle += dt * 80
        global target_color
        if target_color is None and random.randint(1, 1000) == 1000:
            ...
            num_same = 0
            new_c = []
            for xx, cc in zip(target_color, c):
                if xx == cc:
                    new_c.append(cc)
                    num_same += 1
                elif xx > cc:
            ...
            if num_same == 3:
                ...
                target_color = None
            shape.color = new_c
            shape2.color = invert_color(new_c)
