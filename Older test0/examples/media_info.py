...
def print_avbin_info():
    try:
        ...
        print('Using AVbin version %d (FFmpeg r%d)' % (avbin.get_version(), avbin.av.avbin_get_ffmpeg_revision()))
    except:
...
def print_source_info(source):
    ...
    if source.audio_format:
        ...
        print('Audio: %d channel(s), %d bits, %.02f Hz' % (af.channels, af.sample_size, af.sample_rate))
    ...
    if source.video_format:
        ...
        print('Video: %dx%d at aspect %r (displays at %dx%d), %s fps' % (vf.width, vf.height, vf.sample_aspect, display_width, display_height, frame_rate))
