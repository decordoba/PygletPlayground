...
class Game(object):
    def __init__(self, w=20, h=20, px_w=20, px_h=20, title="My game", resizable=False):
        ...
        self.food = [
            GameObject("food",
                       RegularPolygon([self.raw_w / 2, self.raw_h / 2], 4, [255, 0, 0], side=self.px_w, batch=batch_foreground, use_vertices_indices=False, primitive=pyglet.gl.GL_POLYGON))
        ]
        snake_move_logic = DiscreteMoveLogic(steps=[self.px_w, self.px_h])
        self.snake = [
            GameObject("snake0",
                       RegularPolygon([self.raw_w / 4, self.raw_h / 4], 4, [0, 255, 0], side=self.px_w, batch=batch_background),
                       snake_move_logic,
        ...
        self.objects = self.food + self.snake

        self.window = GameWindow(self.objects, batches, self.raw_w, self.raw_h, title, resizable)
        self.key_handler = pyglet.window.key.KeyStateHandler()
